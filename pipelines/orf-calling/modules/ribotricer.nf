...
process PREP_RIBOTRICER {
  ...
  publishDir "${params.outdir}/orf_calls/ribotricer", mode: 'copy', pattern: "prep/**"
...
process RUN_RIBOTRICER {
  ...
  conda "bioconda::ribotricer"
  container "${ params.container_ribotricer ?: 'quay.io/biocontainers/ribotricer:latest' }"
  publishDir "${params.outdir}/orf_calls/ribotricer", mode: 'copy', pattern: "raw/**"
...
process PARSE_RIBOTRICER {
  ...
  publishDir "${params.outdir}/orf_calls/ribotricer", mode: 'copy', pattern: "*"

  input:
  tuple val(meta), path(rawdir)

  output:
  tuple val(meta), path("${meta.id}.ribotricer.orf_calls.tsv"), emit: standardized
  tuple val(meta), path("${meta.id}.ribotricer.orf_calls.bed12"), emit: bed12
...
tsv = Path("${meta.id}.ribotricer.orf_calls.tsv")
bed = Path("${meta.id}.ribotricer.orf_calls.bed12")
...
PY
  ...
  stub:
  """
  echo -e "sample_id\ttool\tchrom\tstart\tend\tstrand\tframe\ttranscript_id\torf_id\tscore\tpval\tqval\textra_json" > ${meta.id}.ribotricer.orf_calls.tsv
  echo -e "chr1\t120\t210\torf|TX1|ribotricer\t0\t+\t120\t210\t0,0,0\t1\t90,\t0," > ${meta.id}.ribotricer.orf_calls.bed12
  cat <<-END_VERSIONS > versions.yml
  "${task.process}":
      parser: stub
  END_VERSIONS
