...
process PREP_RIBOTAPER {
  ...
  publishDir "${params.outdir}/orf_calls/ribotaper", mode: 'copy', pattern: "prep/**"
...
process RUN_RIBOTAPER {
  ...
  publishDir "${params.outdir}/orf_calls/ribotaper", mode: 'copy', pattern: "raw/**"
...
process PARSE_RIBOTAPER {
  ...
  label 'process_low'
  container "ubuntu:22.04"
  publishDir "${params.outdir}/orf_calls/ribotaper", mode: 'copy', pattern: "*"

  input:
  tuple val(meta), path(rawdir)

  output:
  tuple val(meta), path("${meta.id}.ribotaper.orf_calls.tsv"), emit: standardized
  tuple val(meta), path("${meta.id}.ribotaper.orf_calls.bed12"), emit: bed12
...
tsv = Path("${meta.id}.ribotaper.orf_calls.tsv")
bed = Path("${meta.id}.ribotaper.orf_calls.bed12")
...
PY
  ...
  stub:
  """
  echo -e "sample_id\ttool\tchrom\tstart\tend\tstrand\tframe\ttranscript_id\torf_id\tscore\tpval\tqval\textra_json" > ${meta.id}.ribotaper.orf_calls.tsv
  echo -e "chr1\t150\t260\torf|NA|ribotaper\t0\t+\t150\t260\t0,0,0\t1\t110,\t0," > ${meta.id}.ribotaper.orf_calls.bed12
  cat <<-END_VERSIONS > versions.yml
  "${task.process}":
      parser: stub
  END_VERSIONS
