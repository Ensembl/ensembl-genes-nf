...
process PREP_RPBP {
  ...
  publishDir "${params.outdir}/orf_calls/rpbp", mode: 'copy', pattern: "prep/**"
...
process RUN_RPBP {
  ...
  publishDir "${params.outdir}/orf_calls/rpbp", mode: 'copy', pattern: "raw/**"
...
process PARSE_RPBP {
  ...
  label 'process_low'
  container "ubuntu:22.04"
  publishDir "${params.outdir}/orf_calls/rpbp", mode: 'copy', pattern: "*"

  input:
  tuple val(meta), path(rawdir)

  output:
  tuple val(meta), path("${meta.id}.rpbp.orf_calls.tsv"), emit: standardized
  tuple val(meta), path("${meta.id}.rpbp.orf_calls.bed12"), emit: bed12
...
tsv = Path("${meta.id}.rpbp.orf_calls.tsv")
bed = Path("${meta.id}.rpbp.orf_calls.bed12")
...
PY
  ...
  stub:
  """
  echo -e "sample_id\ttool\tchrom\tstart\tend\tstrand\tframe\ttranscript_id\torf_id\tscore\tpval\tqval\textra_json" > ${meta.id}.rpbp.orf_calls.tsv
  echo -e "chr1\t300\t420\torf|NA|rpbp\t0\t+\t300\t420\t0,0,0\t1\t120,\t0," > ${meta.id}.rpbp.orf_calls.bed12
  cat <<-END_VERSIONS > versions.yml
  "${task.process}":
      parser: stub
  END_VERSIONS
