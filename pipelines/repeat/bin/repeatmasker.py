...
def run_repeatmasker(
    ...
    logger.info("repeatmasker path is %s",repeatmasker_bin)
...
def _multiprocess_repeatmasker(  # pylint: disable=too-many-locals
    ...
    logger.info("region_name is %s",region_name)
#try:
    seq = get_sequence(
...
#except Exception as e:
    ...
    logger.info("seq is %s",seq)
    slice_file_name = f"{region_name}.rs{start}.re{end}"
    logger.info ("slice_file_name_is %s",slice_file_name)
    ...
    logger.info("Here is the repeatmasker command used",repeatmasker_cmd)
