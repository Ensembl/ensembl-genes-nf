...
from hpp.config import (
    MappingConfig,
    MappingStatus,
    CANONICAL_DONOR_SITES,
    CANONICAL_ACCEPTOR_SITES,
)
from hpp.gff3_parser import parse_gff3
from hpp.gff3_writer import write_genes_to_gff3
from hpp.fasta_handler import FastaHandler

from hpp.stages.synteny import create_syntenic_map, get_synteny_statistics
from hpp.stages.mapping import (
    map_genes,
    GeneMappingResult,
    resolve_paralog_clusters_synteny_aware,
)
from hpp.stages.internal_realign import targeted_internal_realign
from hpp.stages.refinement import refine_mappings
from hpp.stages.gap_filling import fill_synteny_gaps
from hpp.stages.sex_chromosome import detect_sex_chromosomes, SexChromosomeMap

from hpp.validation.structural import (
    validate_mapped_genes, GeneValidation, StructuralValidator, TranscriptValidation
)
from hpp.validation.protein import GeneProteinQC, run_protein_qc
from hpp.validation.statistics import (
...
class PangenomeMappingPipeline:
    ...
    def __init__(self, config: MappingConfig):
        ...
        # Optional externally-produced PAF (e.g. from a standalone WGA module).
        # When set, synteny detection parses this instead of running minimap2.
        self.external_paf: Optional[Path] = None
    ...
    def _run_synteny_detection(self):
        ...
        if self.external_paf is not None:
            # A standalone WGA module already produced the alignment; create_syntenic_map
            # detects the existing cs-tagged PAF and parses it instead of running minimap2.
            paf_output = Path(self.external_paf)
        else:
    ...
    def _run_missed_locus_recovery(
        ...
        from hpp.stages.mapping import FeatureMapper
    ...
    def _run_validation(self):
        """Run structural validation (initial validate + rescues + protein QC).

        Kept as the monolith entry point; the staged pipeline calls
        :meth:`_validate_initial` and :meth:`_run_rescues_and_protein` as two
        separate processes. Behaviour is identical either way.
        """
        start = time.time()
        self._validate_initial()
        self._run_rescues_and_protein()
    ...
    def _validate_initial(self):
        """Compute initial structural validation for all mapped genes."""
    ...
    def _run_rescues_and_protein(self):
        """Run rescue passes, re-validate, and protein QC (mutates mapped genes)."""
        with FastaHandler(self.config.target_fasta) as fasta:
