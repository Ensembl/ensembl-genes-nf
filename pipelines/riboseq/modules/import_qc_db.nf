process IMPORT_QC_DB {
    ...
    def metrics_args = metric_files.findAll { value -> value }.collect { value -> value.toString() }.join(' ')
    def artifacts_args = artifact_files.findAll { value -> value }.collect { value -> value.toString() }.join(' ')
    def rule_set_args = rule_set_files.findAll { value -> value }.collect { value -> value.toString() }.join(' ')
    def qc_eval_args = qc_eval_files.findAll { value -> value }.collect { value -> value.toString() }.join(' ')
    def gate_selection_args = gate_selection_files.findAll { value -> value }.collect { value -> value.toString() }.join(' ')
    """
    import_qc_db.py \\
      --db qc.duckdb \\
