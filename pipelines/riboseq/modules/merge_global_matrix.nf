...
process MERGE_GLOBAL_MATRIX_PARTITIONED {
    ...
    output:
    tuple val(partition), path("global.${partition}_matrix.parquet"), emit: matrix
    tuple val(partition), path("global.${partition}_counts"), emit: counts, optional: true
    tuple val(partition), path("global.${partition}_reads.parquet"), emit: reads, optional: true
    tuple val(partition), path("global.${partition}_samples.parquet"), emit: samples_parquet, optional: true
    tuple val(partition), path("global.${partition}_studies.parquet"), emit: studies_parquet, optional: true
    tuple val(partition), path("global.${partition}_tombstones.parquet"), emit: tombstones, optional: true
    tuple val(partition), path("global.${partition}_reads.fasta"), emit: fasta
    ...
    merge_global_matrix.py \\
        ...
        --sparse-read-bucket-size ${sparse_read_bucket_size} \\
        --partition ${partition} \\
        --partition-ordinal ${partition_ordinal} \\
        --partition-stride ${partition_stride} \\
        ${fasta_flag}
    ...
    stub:
    """
    mkdir -p global.${partition}_matrix.parquet
    mkdir -p global.${partition}_counts
    touch global.${partition}_reads.parquet
    touch global.${partition}_samples.parquet
    touch global.${partition}_studies.parquet
    touch global.${partition}_tombstones.parquet
    touch global.${partition}_reads.fasta
    touch global.${partition}_metadata.parquet
    echo '{"version": "1.0", "matrix_format": "sparse-parquet", "status": "stub"}' > global.${partition}_matrix_manifest.json
    echo '{"version": "1.0", "matrix_format": "sparse-parquet"}' > global.${partition}_config.json
