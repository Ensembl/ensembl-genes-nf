...
def load_gff3_as_annotation(path: str, label: str) -> Annotation:
    ...
    def norm_parent_gene_id(x: Optional[str]) -> Optional[str]:
        if not x:
            return None
        # Parent may be like gene:ENSG... or just ENSG...
        # Keep full token if it's already an Ensembl-style prefixed ID; many Ensembl
        # files use the prefixed form consistently across gene and transcript.
        # For robustness, if multiple parents (comma), take the first for gene linkage.
        tok = x.split(',')[0]
        # Some sources encode Parent=gene:ID; others just ID. We keep as-is but
        # special-case obviously generic prefixes to strip only for determining linkage.
        if ':' in tok and tok.split(':',1)[0].lower() in ('gene','locus'):
            return tok  # keep prefixed gene:ID form as the gene identifier
        return tok
    ...
    with _open_maybe_gzip(path) as fh:
        for line in fh:
            ...
            if ftype_l in TRANSCRIPT_TYPES:
