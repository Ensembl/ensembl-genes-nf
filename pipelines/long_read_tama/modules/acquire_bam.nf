process ACQUIRE_LONG_READ_BAM {
    ...
    test "\$(md5sum "\${cache_file}" | awk '{print \$1}')" = "${expected_md5}"
    ...
    for i in "\${!URIS[@]}"; do
      ...
      if [ ! -f "\${side_cache}" ]; then curl --fail --location --retry 3 --output "\${side_cache}.part" "\${URIS[i]}"; test "\$(md5sum "\${side_cache}.part" | awk '{print \$1}')" = "\${MD5S[i]}"; mv "\${side_cache}.part" "\${side_cache}"; fi
      test "\$(md5sum "\${side_cache}" | awk '{print \$1}')" = "\${MD5S[i]}"
