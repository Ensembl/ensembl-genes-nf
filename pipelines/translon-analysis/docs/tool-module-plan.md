...
| ORFquant | R/SaTAnn ORF annotation and quantification | genome BAM plus read-length/P-site offsets | R-generated ORF/quantification tables | custom Bioconda-derived image, R compatibility wrapper, dynamic offset-file input, and parser implemented | mini fixture, runner + standardiser exit 0; zero-call native result recorded explicitly |
...
| iRibo | periodicity/scoring caller | genome BAM plus annotation | `translated_orfs.csv`/GFF3 | source-pinned container, shardable candidate/profile stages, and one global translatome stage; known sparse-input failure is recorded as a native zero-call result | adequate real-data smoke, standardised output |
...
| RiboTIE | learned/model-based ORF scoring | transcriptome BAM + reference | prediction CSV/GTF | explicit GPU-only runner, CUDA image scaffold, and runtime CUDA check added; real smoke still required | source-pinned CUDA container, one-GPU allocation, real call, standardised output |
