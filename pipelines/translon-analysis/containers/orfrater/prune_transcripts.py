...
if opts.pseudogenes:
    with open(opts.pseudogenes, 'r') as inpseudo:
...
def _get_tid_info(tup):
    ...
    (chrom, strand) = tup
    inbams = [pysam.AlignmentFile(infile, 'rb') for infile in opts.bamfiles]
    ...
    tid_summary = pd.DataFrame(
        ...
        index=pd.Index(list(bedlinedict[(chrom, strand)].keys()), name='tid'))
...
tid_summary = pd.concat(workers.map(_get_tid_info, list(bedlinedict.keys())))
workers.close()
...
workers = mp.Pool(opts.numproc)
mm_df = workers.map(_find_mm_in_range, list(range(npart)))
workers.close()
...
if pseudos.size > 0:
    ...
    workers = mp.Pool(opts.numproc)
    mm_df = workers.map(_find_kept_mm_in_range, list(range(npart)))
    workers.close()
    mm_df_res = mm_df[0]
    for df in mm_df[1:]:
