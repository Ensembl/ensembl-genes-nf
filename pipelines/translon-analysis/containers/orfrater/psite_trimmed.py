...
def _map_start_sites(chrom_strand):
...
offset_tallies = sum(workers.map(_map_start_sites, list(gcoorddict.keys()))).astype(np.float64)  # convert to float, or at least int64, for convolution
workers.close()
