...
process BUSCO_CORE_METAKEYS {
    ...
    busco_metakeys_patch.py -db ${dbname} -file ${summary_file} -output_dir "${params.outDir}/$publish_dir/" -host ${params.host} -port ${params.port} -user ${params.user}  -password ${params.password} -species_id ${species_id} -run_query true
    """
