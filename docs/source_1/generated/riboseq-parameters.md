...
| `star_index` | string |  | no | Path to STAR genome index directory |
| `gtf` | string |  | no | Path to GTF annotation file |
| `fasta` | string |  | no | Path to reference genome FASTA file (required for RiboWaltz P-site analysis) |
| `ribometric_annotation` | string |  | no | Path to RiboMetric annotation file (optional, for RiboMetric QC) |
| `chrom_sizes_file` | string |  | no | Path to chromosome sizes file |
...
| `fetch` | boolean | False | no | Enable data fetching from SRA |
...
| `use_architecture_detection` | boolean | False | no | Use getRPF architecture detection for adapter finding |
| `mismatches` | integer | 3 | no | Maximum number of mismatches allowed in alignment |
| `alignment_type` | string (EndToEnd, Local) | EndToEnd | no | STAR alignment type |
